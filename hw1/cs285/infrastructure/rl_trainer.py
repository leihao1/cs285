...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        ## 1TODO initialize all of the TF variables (that were created by agent, etc.)
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            training_returns = self.collect_training_trajectories(itr,
                                ...
                                self.params['batch_size']) # 1TODO implement this function below
            paths, envsteps_this_batch, train_video_paths = training_returns
            self.total_envsteps += envsteps_this_batch

            # relabel the collected obs with actions from a provided expert policy
            if relabel_with_expert and itr>=start_relabel_with_expert:
                paths = self.do_relabel_with_expert(expert_policy, paths) # 1TODO implement this function below

            # add collected data to replay buffer
            self.agent.add_to_replay_buffer(paths)

            # train agent (using sampled data from replay buffer)
            self.train_agent() # 1TODO implement this function below
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, batch_size):
        ...
        # 1TODO decide whether to load training data or use
        ...
        if itr == 0:
            print('first iteration, load expert from %s' % load_initial_expertdata)
            with open(load_initial_expertdata,'rb') as file:
                loaded_paths = pickle.load(file)
            return loaded_paths, 0 , None

        # 1TODO collect data to be used for training
        ...
        if self.log_video:
            ...
            # 1TODO look in utils and implement sample_n_trajectories
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            # 1TODO sample some data from the data buffer
            # HINT1: use the agent's sample function
            # HINT2: how much data = self.params['train_batch_size']
            ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch = self.agent.sample(self.params['train_batch_size'])

            # 1TODO use the sampled data for training
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        # 1TODO relabel collected obsevations (from our policy) with labels from an expert policy
